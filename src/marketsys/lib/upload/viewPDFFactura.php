...
<head>
	...
	<script type="text/javascript">
			...
			form_data.append("file", file_data);
				$.ajax({
					...
					success: function(data)
					{
						...
						parent.document.getElementById('filePDF').value=res;
						document.getElementById('imagenReclamo').src = '../respaldos_compras/pdf/archivo-subido.png';
					}
				});
		...
		function eliminarArchivo(){
			document.getElementById('imagenReclamo').src = "../respaldos_compras/pdf/archivo-no-disponible.png";
		...
		function previsualizarArchivo() {
			if (document.getElementById('tituloImage').innerHTML!='No Disponible'){
				parent.mostrarPrevisualizar('../respaldos_compras/pdf/'+document.getElementById('tituloImage').innerHTML);
			}else{alert('No se puede realizar la descarga, por favor seleccione una imagen.')}
		}
		
		function descargaArchivo() {
		    if (document.getElementById('tituloImage').innerHTML!='No Disponible'){
				location.href = "../respaldos_compras/pdf/descargarArchivoPredios.php?file="+
	...
	<table>
		<tr>
			<td style="width: 100%; text-align: center;">
				<div id="targetLayer">
					<img id="imagenReclamo" src="../respaldos_compras/pdf/archivo-no-disponible.png" width="100px" height="100px" />
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td style="text-align: center;">
				...
				<input type="file" id="imageFoto" name="imageFoto" accept="application/pdf" class="upload" 
